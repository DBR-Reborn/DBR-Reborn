...
void info() {
message("help", "This spell allows a Voidwalker to magically reach into someones soul and bring out some of their dormant anger, raising their attack and weapon skills. This magic may be cast on any target a total of two times.", this_player());
}
...
if ((int)caster->query("oberon fury #") >= 2) {
    message("info", (string)caster->query_cap_name() +
	    " can not get more angry.", caster);
    caster->add_mp(props["mp cost"]);
    remove();
