...
void create() {
    ::create();
    set_property("name","summon elemental titan");
    ...
    set_property("observer message","$C begins to magically mold a mythical figurine.");
...
"time to cast.  The spell must be cast on a mythical figurine, which is "
...
else { start_work(at, caster, 7200 + 900 * power + (caster->query_level()*333) , power);}
  return;
}
...
void finish_work(object caster, object at, int power) {
  ...
  message("info", "The mythical figurine transforms into a "+props["element"]+
	  " titan!", caster);
  message("info", (string)caster->query_cap_name() + " utters some "+
	  "magical incantations, and an mythical figurine "+
	  "transforms into a titan of "+props["element"]+".",
	  environment(caster), ({ caster }) );
  at->remove();
  seteuid(getuid());
  //ob = new("/wizards/diewarzau/obj/pet/elemental_dragon");
   ob = new("/std/spells/summon/ele/elemental_titan"); 
//ADD
    zlevel = caster->query_level()-random(11);
if(zlevel <= 20) {zlevel=20;}
    ob->set_level(zlevel);
    ob -> set_property("new desc", "["+zlevel+"] "+capitalize(props["element"])+" Elemental Titan");
