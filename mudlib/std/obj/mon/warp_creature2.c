...
void die(object obi) {
	...
	message("info","%^BLUE%^A portal opens! It will close as soon as reality "+
		"stabalizes.",tmp);
dmg1= this_object()->query_attackers()[0]->query_level()+20+random(11);

message("info", "The creature dies and a stronger version appears",tmp);
 ob=new("/std/obj/mon/warp_creature3");
ob->set_level(dmg1);
ob->set_short("["+dmg1+"] Warp Creature");
ob->set_melee_damage(([ "crushing" : (125+dmg1)*10, "electricity" : (125+dmg1)*10, "vacuum" : (125+dmg1)*10, "stress" : (125+dmg1)*10, "aether" : (125+dmg1)*15, "unholy" : (125+dmg1)*10 ]));
ob->set_skill("melee", 14*dmg1);
ob->set_skill("martial arts", 10*dmg1);
ob->move(environment());
