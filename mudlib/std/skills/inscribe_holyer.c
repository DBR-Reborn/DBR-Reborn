...
message("info", "%^BOLD%^%^YELLOW%^It glows with the brilliance of a Masterpiece!%^RESET%^", from);
 ...
 tmp = at->query_property("extra long");
    if(pointerp(tmp)) tmp += ({ "A Masterpiece %^BOLD%^%^WHITE%^Holyer%^RESET%^ rune is inscribed on the weapon." });
    else if(stringp(tmp)) tmp = ({ tmp, "A Masterpiece %^BOLD%^%^WHITE%^Holyer%^RESET%^ rune is inscribed on the weapon." });
  else tmp = ({ "A Masterpiece %^BOLD%^%^WHITE%^Holyer%^RESET%^ rune is inscribed on the weapon." });
}
//END
