...
set_property("prereq", "void edge");
return;
}

void info() {
message("help", "This is a sickeningly powerful spell that imbues tremendous void power into any weapon, a combination of massive vacuum, aether, and chaotic energy. To cast this however requires a highly considerable amount of mp and its duration is lower than that of most other spells of this type. It can be stacked on a weapon a total of two times, and due to its extreme power cannot be used in conjunction with the arcane-imbuer spell 'bind enchantment'.", this_player());
}
...
message("info", "%^B_RED%^%^BOLD%^%^BLACK%^You imbue the weapon with mortifying void power!%^RESET%^", caster);
ob->set_extra_wc(([ "vacuum" : 15*power ]));
