...
set_property("caster message", "You cast %^BOLD%^%^BLACK%^vacuum edge %^RESET%^at $T!");
set_property("target message", "");
set_property("observer message", "$C casts %^BOLD%^%^BLACK%^vacuum edge %^RESET%^at $T.");
...
void info() {
message("help", "This spell imbues vacuum powers into a weapon, allowing its wielder to strike its foes with increased vacuum elemental energy for the spells duration.", this_player());
}
...
if(check_stack(at, ob)) {
	message("info", "You imbue the weapon with vacuum.", caster);
	ob->set_extra_wc(([ "vacuum" : 3*power ]));
	ob->set_shadow_name("vacuum edge");
        ob->start_shadow(at, props["duration"], "%^BOLD%^%^BLACK%^A vacuum edge spell wears off.%^RESET%^");
  }    
	else{	
